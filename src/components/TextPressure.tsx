...
const TextPressure = ({
  ...
  useEffect(() => {
    ...
    const debouncedSetSize = (..._args: any[]) => { clearTimeout(timeout); timeout = setTimeout(() => setSize(), 100); };
    debouncedSetSize();
    window.addEventListener('resize', debouncedSetSize);
    return () => { window.removeEventListener('resize', debouncedSetSize); clearTimeout(timeout); };
  }, [setSize]);
  ...
  return (
    <div ref={containerRef} className={`w-full h-full relative select-none ${className}`}>
      ...
      <div
        ref={titleRef}
        className={`absolute inset-0 flex items-center justify-center whitespace-nowrap ${flex ? 'flex' : ''} ${stroke ? 'text-pressure-stroke' : ''}`}
        style={{
